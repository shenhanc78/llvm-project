...
// Pass for finding hash of basic blocks to common.
...
class AdjustStack : public BinaryFunctionPass {
private:
  std::vector<std::string> Spec;
public:
  explicit AdjustStack(const cl::opt<bool> &PrintPass,
                     cl::list<std::string> &Spec)
      : BinaryFunctionPass(PrintPass), Spec(Spec) {}
  
  int getNumBytes() const;
  const char *getName() const override { return "adjust-stack"; }
  Error runOnFunctions(BinaryContext &BC) override;
};
...
public:
  ...
  Error globalizeSymbolsBeforeOutline(BinaryContext &BC, BinaryFunction *Function, std::vector<BinaryBasicBlock *> Blocks, int OutlineBlockCounter, std::string blockHash) const;
  Error globalizeSymbolsAfterOutline(BinaryContext &BC, BinaryFunction *Function, std::vector<BinaryBasicBlock *> Blocks, int OutlineBlockCounter, std::string blockHash) const;
  ...
  Error outlineAnyOtherBB(std::vector<BinaryFunction *> Functions, std::string blockHash, int &RedundantBlockCount, int &TotalBlocksOutlined, int SIZE_THRESHOLD, int &countRemaining) const;
