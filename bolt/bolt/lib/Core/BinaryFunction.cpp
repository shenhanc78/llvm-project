...
void BinaryFunction::removeBB(const MCCodeEmitter *Emitter, BinaryBasicBlock *CurBB) {
  DenseSet<const BinaryBasicBlock *> InvalidBBs;
  unsigned Count = 0;
  uint64_t Bytes = 0;

  InvalidBBs.insert(CurBB);
  ++Count;
  Bytes += BC.computeCodeSize(CurBB->begin(), CurBB->end(), Emitter);
...
bool BinaryFunction::postProcessIndirectBranches(
  ...
  auto addUnknownControlFlow = [&](BinaryBasicBlock &BB) {
    ...
    //                   << " for " << BB.getName() << "\n");
    outs() << "BOLT-DEBUG: adding unknown control flow in " << *this
                      << " for " << BB.getName() << "\n";
