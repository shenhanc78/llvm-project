...
public:
  ...
  InstructionListType createConditionalJumpToOutline(const MCSymbol *Target, MCContext *Ctx, MCPhysReg IndirectJumpReg) const override {
    InstructionListType Code;

    Code.emplace_back(MCInstBuilder(X86::CMP64ri8)
                     .addReg(IndirectJumpReg) // Compare against %rbx
                     .addExpr(MCSymbolRefExpr::create(Target, MCSymbolRefExpr::VK_None, *Ctx)));
    ...
    Code.emplace_back(MCInstBuilder(X86::JMP64r)
                             .addReg(IndirectJumpReg));

    return Code;
  }

  InstructionListType createOutlinerToRedirect() const override {
    InstructionListType Code;
  ...
  InstructionListType createJmpToOutlinerRemainBB(const MCSymbol *Target, MCContext *Ctx) const override {
    InstructionListType Code;             
  ...
  MCPhysReg getIndirectJmpReg(BinaryBasicBlock *CurBB) {

   const MCInst &LastInst = CurBB->back();

    // Check if the last instruction is an indirect jump (e.g., JMP64r for x86_64)
    if (LastInst.getOpcode() == X86::JMP64r || LastInst.getOpcode() == X86::JMP32r) {
        // Ensure it has at least one operand and that the operand is a register
        if (LastInst.getNumOperands() > 0 && LastInst.getOperand(0).isReg()) {
            // Fetch the register used in the indirect jump
            MCPhysReg IndirectJumpReg = LastInst.getOperand(0).getReg();
            return IndirectJumpReg;
        }
    }
  ...
  bool isEndingWithCondJump(BinaryBasicBlock *CurBB) {
    if (CurBB->empty())
        return false;
    ...
    //     const MCExpr *Expr = LastInst.getOperand(0).getExpr();

    //     if (const MCSymbolRefExpr *SymExpr = dyn_cast<MCSymbolRefExpr>(Expr)) {
    //       // outs() << "TargetName: " << SymExpr->getSymbol().getName() << "\n";
    //       return true;
    //     }
    // }
  ...
  bool isEndingWithUnCondJump(BinaryBasicBlock *CurBB) {
    if (CurBB->empty())
        return false;
  ...
  bool isJumpThread(BinaryBasicBlock *CurBB) {
    if (CurBB->empty())
        return false;

    bool do_once = false;

    bool uncondJmp = false;
  ...
  bool hasCFI(BinaryBasicBlock *CurBB) {
  ...
  bool isEndingWithCFI(BinaryBasicBlock *CurBB) 
  {
    if (CurBB->empty())
        return false;
  ...
  bool isPrologue(BinaryBasicBlock *CurBB)
  {
    ...
    for (auto II = CurBB->begin(); II != CurBB->end(); ++II) 
    {
        ...
        if (isPush(Inst))
        {
            if ((Inst.getNumOperands() < 2 && Inst.getOperand(0).isReg() && Inst.getOperand(0).getReg() == X86::RBP))
            {
                return true;
            }
        }

        // if (Inst.getOpcode() == X86::MOV64rr || Inst.getOpcode() == X86::MOV32rr || Inst.getOpcode() == X86::MOV16rr || Inst.getOpcode() == X86::MOV8rr)
        // {
        //     if (Inst.getNumOperands() == 2 && Inst.getOperand(0).isReg() && Inst.getOperand(0).getReg() == X86::RBP && Inst.getOperand(1).isReg() && Inst.getOperand(1).getReg() == X86::RSP)
        //     {
        //         return true;
        //     }
        // }
  ...
  bool SplitPrologue(MCInst &Inst)
  {
  ...
  bool SplitJmpBB(MCInst &Inst)
  {

    if (isUnconditionalBranch(Inst) || isConditionalBranch(Inst))
    {
  ...
  bool isEpilogue(BinaryBasicBlock *CurBB)
  {
  ...
  void setStackPointerIncrement(BinaryBasicBlock *CurBB) {
    ...
    for (auto II = CurBB->begin(); II != CurBB->end(); ++II) 
    {
        ...
        if (isPush(Inst))
        {
            if ((Inst.getNumOperands() < 2 && Inst.getOperand(0).isReg() && Inst.getOperand(0).getReg() == X86::RBP))
            {
                InstructionListType NewCode;

                

                NewCode.emplace_back(MCInstBuilder(X86::SUB64ri8)
                              .addReg(X86::RSP)    
                              .addReg(X86::RSP)
                              .addImm(padStack())); 

                // NewCode.emplace_back(MCInstBuilder(X86::AND64ri32)
                //               .addReg(X86::RSP)    
                //               .addReg(X86::RSP)
                //               .addImm(-16)); 
                ...
                II = CurBB->replaceInstruction(II, NewCode);
  ...
  void setStackPointerDecrement(BinaryBasicBlock *CurBB) 
  {
    ...
    for (auto II = CurBB->begin(); II != CurBB->end(); ++II) 
    {
        ...
        if (isPop(Inst))
        {
            if (Inst.getNumOperands() < 2 && Inst.getOperand(0).isReg() && Inst.getOperand(0).getReg() == X86::RBP)
            {
              ...
              NewCode.emplace_back(MCInstBuilder(X86::ADD64ri8)
                            .addReg(X86::RSP)    
                            .addReg(X86::RSP)
                            .addImm(padStack())); 
  ...
  void updateStackFrameRegOffset(BinaryBasicBlock *CurBB) {
    ...
    for (auto II = CurBB->begin(); II != CurBB->end(); ++II) 
    {
        ...
        // if (isPush(Inst))
        // {
        //     if ((Inst.getNumOperands() > 0 && Inst.getOperand(0).isReg() && Inst.getOperand(0).getReg() == X86::RBP))
        //     {
        //         continue;
        //     }
        // } 
        ...
        for (unsigned i = 0; i < Inst.getNumOperands(); i++) {
            ...
            if (Operand.isReg() && Operand.getReg() == X86::RBP)
            {
                // outs() << "Reg: " << Operand.getReg() << " (X86::RBP) \n";
                // if(Inst.getOperand(i+1).isImm())
                //   outs() << "ImmValue: " << Inst.getOperand(i+1).getImm() << "\n";

                if (i+3 < Inst.getNumOperands())
                {
                  // outs() << "i+3 " << i+3 << "\n";
                  if(Inst.getOperand(i+3).isImm() && Inst.getOperand(i+3).getImm() > 0)
                  {
                    // outs() << "OldImmValue: " << Inst.getOperand(i+3).getImm() << "\n";
                    int OldBytes = Inst.getOperand(i+3).getImm();
                    Inst.getOperand(i+3).setImm(OldBytes+16);
                    // outs() << "NewImmValue: " << Inst.getOperand(i+3).getImm() << "\n";
                  }
        ...
        //         if (Reg == X86::RBP) // (Reg == X86::RSP || Reg == X86::RBP)
        //         { 
        //             if (i - 1 < Inst.getNumOperands() && Inst.getOperand(i - 1).isImm() && Inst.getOperand(i - 1).getImm()>1) {
        //                 outs() << "Other: Immediate Value (RBP): " << Inst.getOperand(i + 1).getImm() << "\n";
        //                 int OldBytes = Inst.getOperand(i + 1).getImm();
        //                 int newBytes = OldBytes + 16;
        //                 Inst.getOperand(i + 1).setImm(newBytes);
        //                 outs() << "Other: Updated Immediate Value (RBP): " << Inst.getOperand(i + 1).getImm() << "\n";
        //             }
        ...
        //         // {
        //         //     if (i + 1 < Inst.getNumOperands() && Inst.getOperand(i + 1).isImm() && Inst.getOperand(i + 1).getImm()!=1) {
        //         //         outs() << "Other: Immediate Value (RSP): " << Inst.getOperand(i + 1).getImm() << "\n";
        //         //         int OldBytes = Inst.getOperand(i + 1).getImm();
        //         //         int newBytes = OldBytes + 16;
        //         //         Inst.getOperand(i + 1).setImm(newBytes);
        //         //         outs() << "Other: Updated Immediate Value (RSP): " << Inst.getOperand(i + 1).getImm() << "\n";
        //         //     }
        //         // }
        //     }
        // }
  ...
  bool hasStackFrameReg(BinaryBasicBlock *CurBB) 
  {
    ...
    for (auto II = CurBB->begin(); II != CurBB->end(); ++II) 
    {
        ...
        for (unsigned i = 0, e = Inst.getNumOperands(); i < e; ++i) {
            ...
            if (Operand.isReg()) {
                unsigned Reg = Operand.getReg();
                if (Reg == X86::RBP || Reg == X86::RSP) // (Reg == X86::RSP || Reg == X86::RBP)
                { 
                    return true;
                }
            }
  ...
  bool hasIndirectCalltoRAX(BinaryBasicBlock *CurBB) 
  {
    if (CurBB->empty())
        return false;

    const MCInst &Inst = CurBB->back();
    ...
    if (isCall(Inst))
    {
      ...
      for (unsigned i = 0, e = Inst.getNumOperands(); i < e; ++i) {
        ...
        if (Operand.isReg()) {
            unsigned Reg = Operand.getReg();
            if (Reg == X86::RAX && Inst.getNumOperands() > 4)
            { 
                return true;
            }
        }
  ...
  InstructionListType replaceFunctionWithCallAndReturn (const MCSymbol *Destination, MCContext *Ctx) const override {
    InstructionListType Code;

    Code.emplace_back(MCInstBuilder(X86::CALL64pcrel32)
                            .addExpr(MCSymbolRefExpr::create(Destination, MCSymbolRefExpr::VK_PLT, *Ctx)));

    Code.emplace_back(MCInstBuilder(X86::RET64));

    return Code;
  }

  InstructionListType createRedirectToSuccessor(const MCSymbol *Destination, MCContext *Ctx) const override {
    InstructionListType Code;

    Code.emplace_back(MCInstBuilder(X86::JMP_4)
                            .addExpr(MCSymbolRefExpr::create(Destination, MCSymbolRefExpr::VK_None, *Ctx)));

    return Code;
  }

  bool hasCallInst(BinaryBasicBlock *CurBB) {
  ...
  int getStackFrameOffset(BinaryFunction *Function, BinaryBasicBlock *CurBB) 
  {
    if (CurBB->empty())
        return false;
    ...
    for (BinaryBasicBlock *BB : Blocks) 
    {
        ...
        for (auto II = BB->begin(); II != BB->end(); ++II) 
        {
            ...
            if (isPush(Inst))
            {
                if (!firstRBP)
                {
                  if ((Inst.getOpcode() == X86::PUSH64r && Inst.getNumOperands() > 0 && Inst.getOperand(0).isReg() && Inst.getOperand(0).getReg() == X86::RBP))
                  {
                      offset -= 8;
                      firstRBP = true;
                  }
            ...
            else if ((Inst.getOpcode() == X86::ADD64ri8 || Inst.getOpcode() == X86::ADD64ri32))
            {
                if (firstRBP)
                {
                  if (Inst.getNumOperands() > 2 && Inst.getOperand(0).isReg() && Inst.getOperand(0).getReg() == X86::RSP &&  Inst.getOperand(2).isImm())
                  {
            ...
            else if (Inst.getOpcode() == X86::SUB64ri8 || Inst.getOpcode() == X86::SUB64ri32)
            {
                if (firstRBP)
                {
                  if (Inst.getNumOperands() > 2 && Inst.getOperand(0).isReg() && Inst.getOperand(0).getReg() == X86::RSP &&  Inst.getOperand(2).isImm())
            ...
            else if (Inst.getOpcode() == X86::LEA64r && Inst.getOperand(0).isReg() && Inst.getOperand(0).getReg() == X86::RSP) 
            {

              if (firstRBP)
              {
                  if (Inst.getNumOperands() < 5 && Inst.getOperand(4).isImm())
                  {
                      offset -= Inst.getOperand(4).getImm();
                  }
              }
            }
        }
  ...
  // int getStackAlign(int originalBytes) {
  //     int misalignment = (originalBytes + 8) % 16;
  //     if (misalignment == 0) {
  //         return originalBytes;  // Already aligned
  //     }
  //     return originalBytes + (16 - misalignment);  // Add padding
  // }

  
  

  int isStackPointerDecrement(BinaryBasicBlock *CurBB) {
    ...
    for (auto II = CurBB->begin(); II != CurBB->end(); ++II) 
    {
        ...
        if (isPop(Inst))
        {
          if (Inst.getOperand(0).isReg() && Inst.getOperand(0).getReg() == X86::RBP)
          {
            return 1;
          }
        }
  ...
  int isStackPointerIncrement(BinaryBasicBlock *CurBB) {
    ...
    for (auto II = CurBB->begin(); II != CurBB->end(); ++II) 
    {
        ...
        //     {
        //       do_once = true;
        //       if (Inst.getOperand(2).getImm()!=8)
        //         return Inst.getOperand(2).getImm();
        //       return -1;
        //     }
        // }
    }
  ...
  InstructionListType createRedirectToOutlinerAnyOtherBB(const MCSymbol *Destination, const MCSymbol *Target, int offset, MCContext *Ctx) const override {
    InstructionListType Code;

    Code.emplace_back(MCInstBuilder(X86::PUSH64r)
                            .addReg(X86::R11));
    ...
    Code.emplace_back(MCInstBuilder(X86::MOV64mr)  
                            .addReg(X86::RBP)                         
                            .addImm(1)                                
                            .addReg(0)                                
                            .addImm(16)             
    ...
    Code.emplace_back(MCInstBuilder(X86::POP64r)
                            .addReg(X86::R11));
  ...
  InstructionListType createOutlinerToRedirectAnyOtherBB(int offset) const override {
    InstructionListType Code;       

    Code.emplace_back(MCInstBuilder(X86::JMP64m)          
                              .addReg(X86::RBP) 
                              .addImm(1)        
                              .addReg(0)        
                              .addImm(16)       
  ...
  InstructionListType createRedirectToSuccessorAnyOtherBB() const override {
    InstructionListType Code;

    Code.emplace_back(MCInstBuilder(X86::ADD64ri8)
                            .addReg(X86::RSP)    
                            .addReg(X86::RSP)
                            .addImm(8)); 
  ...
  void createCallToAddress(MCInst &Inst, const uint64_t TargetAddr,
                  MCContext *Ctx) override {
    Inst.setOpcode(X86::CALL64pcrel32);
    Inst.clear();
    Inst.addOperand(MCOperand::createExpr(
        MCConstantExpr::create(TargetAddr, *Ctx)));
  }

  void pushRAX(MCInst &Inst, MCContext *Ctx) override
  {
    Inst.setOpcode(X86::PUSH64r);
    Inst.clear();
    Inst.addOperand(MCOperand::createReg(X86::RAX));
  }

  void popRAX(MCInst &Inst, MCContext *Ctx) override
  {
    Inst.setOpcode(X86::POP64r);
    Inst.clear();
    Inst.addOperand(MCOperand::createReg(X86::RAX));
  }
