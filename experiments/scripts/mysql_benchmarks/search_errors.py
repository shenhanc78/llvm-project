#!/usr/bin/env python3
# search_error.py — fast, crash-safe delta-debugging for preserve_none candidate sets
...
TARGET_BUILD = 'thinlto_autofdo_mysql'

# Paths are relative to mysql_benchmarks/
LIVENESS_DATA_DIR = Path(f"../../metrics/liveness_output/{TARGET_BUILD}_liveness_output")
PN_FUNCTIONS_DIR = Path(f"../../metrics/pn_functions/{TARGET_BUILD}_pn_functions")

CANDIDATES_JSON = PN_FUNCTIONS_DIR / "liveness_profdata.json"
GOOD_FUNCTIONS_JSON = Path("../../metrics/pn_functions/mysql_good_functions.json")
BAD_FUNCTIONS_TXT = Path("../../metrics/pn_functions/mysql_bad_functions.txt")
CACHE_PKL = PN_FUNCTIONS_DIR / "tested_cache.pkl"
BACKUP_SUFFIX = ".bak"


# Your make target that consumes CANDIDATES_JSON and crashes
MAKE_TARGET = "test_pn_thinlto_autofdo_mysql"
...
OPTIONAL_SCORED_CANDIDATES_JSON = None 

MAKE_FLAGS = ["-j", f"JOBS={os.cpu_count() or 32}"]
...
def write_json_functions(path: Path, functions: Iterable[str]) -> None:
    ...
    """
    # Write the format the compiler expects: { "functions": { "name": score } }
...
def main():
    ...
    if not source_path.exists():
        ...
        print(f"Please run 'generate_liveness_profdata.py' first.")
        sys.exit(1)
